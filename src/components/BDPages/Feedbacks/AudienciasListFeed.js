...
const AudienciasListFeed = () => {
    ...
    const fetchComments = async () => {
        try {
            const res = await axios.get(`https://justix-back.vercel.app/av_juiz/${id_juiz}`);
    ...
    const fetchAverageRating = async () => {
        try {
            const res = await axios.get(`https://justix-back.vercel.app/juiz_avaliacao/${id_juiz}`);
    ...
    return (
        <ContainerHome>
            <div>
                {id_juiz && (
                    <Link to={`/user/dashboard/audiencias/${id_juiz}/feedback/add`} className={styles.feedbtn}>
...
export default AudienciasListFeed;
